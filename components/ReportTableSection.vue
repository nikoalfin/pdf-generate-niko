<template>
  <section class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="(row, index) in rows" :key="index" class="hover:bg-gray-50 transition">
            ...
            <td class="px-3 py-2.5 text-center space-x-1">
              <button @click="openDetail(row)" class="px-2 py-1 bg-blue-100 text-blue-600 font-semibold rounded hover:bg-blue-200 transition">View</button>
              <button @click="openDelete(index, row)" class="px-2 py-1 bg-red-100 text-red-600 font-semibold rounded hover:bg-red-200 transition">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <!-- Detail Modal -->
    <div v-if="isDetailOpen" class="fixed inset-0 z-40 flex items-center justify-center bg-black/40" @click.self="closeDetail">
      <div class="w-full max-w-lg bg-white rounded-xl shadow-xl border border-gray-200 p-6">
        <div class="flex items-start justify-between mb-4">
          <div>
            <p class="text-xs font-semibold text-gray-500 uppercase tracking-widest">Detail Laporan</p>
            <h4 class="text-lg font-bold text-gray-900">{{ selectedRow?.judul }}</h4>
          </div>
        ...
        <div class="space-y-3 text-sm">
          <div class="flex items-center justify-between">
            ...
            <span class="font-semibold text-gray-900">{{ selectedRow?.pageSize }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-500">Nominal</span>
            <span class="font-semibold text-gray-900">{{ selectedRow?.nominal }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-500">Tanggal</span>
            <span class="font-semibold text-gray-900">{{ selectedRow?.tanggal }}</span>
          </div>
          <div>
            <span class="text-gray-500">Deskripsi</span>
            <p class="mt-2 text-gray-900 bg-gray-50 border border-gray-200 rounded-lg p-3 min-h-[80px]">
    ...
    <div v-if="isDeleteOpen" class="fixed inset-0 z-40 flex items-center justify-center bg-black/40" @click.self="closeDelete">
